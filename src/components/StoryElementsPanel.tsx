...
type SubTabId =
    ...
    | 'images' | 'audio'
    | 'scenes' | 'imagemaps'
    | 'snippets' | 'animations' | 'menuTemplates' | 'colorPalette';
...
interface StoryElementsPanelProps {
    ...
    // ATL Animation Presets
    onInsertATLPresetAtCursor?: (code: string) => void;
        ...
        <div className="h-full bg-secondary text-primary flex flex-row min-h-0" data-tutorial="story-elements">
            ...
            <div className={`flex-1 min-h-0 ${activeSubTab === 'colorPalette' ? 'overflow-hidden' : 'overflow-y-auto overscroll-contain p-4'}`}>
                ...
                {activeSubTab === 'animations' && (
                    <div>
                        <h2 className="text-lg font-semibold mb-4">ATL Animations</h2>
                        <ATLPresetBrowser onInsertAtCursor={onInsertATLPresetAtCursor} />
                    </div>
                )}
